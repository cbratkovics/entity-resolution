-- Grain: one row per (record_id, batch, batch, model_version, candidate):
...
joined as (
    ...
    left join actuals as a on p.record_id = a.record_id and p.period_key = a.period_key
),

running as (
    select
        *,
        fsum(actual) over (partition by eval_window, candidate, record_id order by period_key range between unbounded preceding and 1 preceding) as run_entity_sum,
        count(actual) over (partition by eval_window, candidate, record_id order by period_key range between unbounded preceding and 1 preceding) as run_entity_cnt,
        fsum(actual) over (partition by eval_window, candidate, source order by period_key range between unbounded preceding and 1 preceding) as run_cohort_sum,
        count(actual) over (partition by eval_window, candidate, source order by period_key range between unbounded preceding and 1 preceding) as run_cohort_cnt
    from joined
),
...
with_baseline as (
    ...
    from running as r
    left join seed_entity as se on r.eval_window = se.eval_window and r.record_id = se.record_id
    left join seed_cohort as sc on r.eval_window = sc.eval_window and r.source = sc.source
),
