-- Grain: one row per (batch, batch, source, model_version, candidate, min_floor):
...
scored as (
    select
        *,
        row_number() over (partition by batch, batch, source, model_version, candidate order by prediction desc, record_id asc) as prediction_rank,
        max(actual) over (partition by batch, batch, source, model_version, candidate) as best_eligible,
        case source
...
select
    ...
    cast(batch as integer) as batch,
    cast({{ period_key('batch', 'batch') }} as integer) as period_key,
