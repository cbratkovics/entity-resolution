-- Grain: one row per (record_id, period_key) for every period a prediction exists for: the
...
history as (
    select
        ...
        lead(as_of_period_key) over (partition by record_id order by dbt_valid_from) as valid_to_period_key,
        dbt_valid_from
    from {{ ref('snp_entity') }}
),
...
matched as (
    ...
    left join history as h
        on
            p.record_id = h.record_id
            and p.period_key > h.valid_from_period_key
            and (h.valid_to_period_key is null or p.period_key <= h.valid_to_period_key)
)

select
    cast(m.record_id as varchar) as record_id,
    cast(m.period_key as integer) as period_key,
    cast(coalesce(m.record_name, c.record_name) as varchar) as record_name,
    cast(coalesce(m.source, c.source) as varchar) as source,
...
from matched as m
left join {{ ref('dim_entity_current') }} as c on m.record_id = c.record_id
