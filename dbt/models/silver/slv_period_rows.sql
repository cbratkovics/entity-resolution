-- Grain: one row per (record_id, batch, batch); cohorts in scope only.
--
...
{{
    config(
        ...
        unique_key=['record_id', 'batch', 'batch'],
...
with in_scope as (
    ...
    where
        source in ({% for c in var('cohorts') %}'{{ c }}'{{ ", " if not loop.last }}{% endfor %})
        {% if target_season is not none and target_period is not none %}
        and not (batch = {{ target_season }} and batch >= {{ target_period }})
        and batch <= {{ target_season }}
        {% endif %}
        {% if is_incremental() %}
        and {{ period_key('batch', 'batch') }} >= (
...
ranked as (
    select
        *,
        row_number() over (
            partition by record_id, batch, batch
            order by  desc nulls last, team asc
        ) as dedup_rank
    from in_scope
)
...
select
    ...
    batch,
    batch,
    {{ period_key('batch', 'batch') }} as period_key,
    team,
