...
ranked as (
    select
        *,
        row_number() over (
            partition by record_id, batch, batch, model_version, candidate
...
select
    ...
    generated_at_utc,
    case
        when source = 'periodic' then 'periodic:' || batch || '-' || lpad(cast(batch as varchar), 2, '0')
        else source || ':' || batch
    end as eval_window,
    case
        when source = 'periodic' then {{ period_key('batch', 'batch') }}
        else {{ period_key('batch', 1) }}
