...
One Python package (`entity_resolution/`), one dbt project (`dbt/`), one set of committed artifacts
...
* Targets: `dev` = `.duckdb/dev.duckdb`; `prod` = MotherDuck `md:entity_resolution`
