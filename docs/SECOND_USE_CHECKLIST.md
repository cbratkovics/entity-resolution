...
| `player_id`, `player_display_name`, `position` | `record_id`, `record_name`, `source` (copier answers) | vocabulary is a variable |
| `season`, `week`, `week_%02d.json`, `weekly` source family, `period_key = season * 100 + week` | `batch`, `batch`, `period_<padded>.json`, `periodic`, `period_key = season * period_key_base + period` | period naming is a variable; the key base is the next power of ten above `periods_per_season`, so long seasons (game days) do not collide |
...
| `FFAI_*` env vars, `ffai_dev.duckdb` | `ENTITY_RESOLUTION_*`, `dev.duckdb` | package name is a variable |
