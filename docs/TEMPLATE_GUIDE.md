...
`entity_resolution/data/loader.py` must keep the `SourceLoader` contract (`entity_resolution/interfaces.py`):

1. `ID_COLUMNS` starts with `record_id`, `record_name`, `source`,
   `batch`, `batch`, `team`; `STAT_COLUMNS` are the raw numeric columns and
   end with the published target ``.
...
6. Set the split seasons and `periods_per_season` in `config.py`; regenerate the mirrors
   (`python -m entity_resolution.config --frontend > frontend/src/lib/project.config.json`) and update
...
`entity_resolution/eval/drift.py` ships with `CALIBRATED = False`; the scheduled job downgrades a
...
- **MotherDuck**: create the database `entity_resolution`; add `MOTHERDUCK_TOKEN` as a repository secret.
- **Hugging Face Space** `/entity-resolution` (Docker SDK, port 7860); add `HF_TOKEN` (write) as a secret; run `ci.yml` with `deploy_space=true` once.
