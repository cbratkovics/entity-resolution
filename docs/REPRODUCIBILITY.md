...
2. **The fix is DuckDB's `threads` setting.** The dev profile reads it from
   `ENTITY_RESOLUTION_DUCKDB_THREADS` (default 4). With `ENTITY_RESOLUTION_DUCKDB_THREADS=1`
...
```bash
export ENTITY_RESOLUTION_DUCKDB_THREADS=1
# before
...
for f in sorted(glob.glob("/tmp/marts_before/*.parquet")):
    ...
    keys = [c for c in a.columns if c in ("record_id", "batch", "batch", "model_version", "candidate", "cohort", "min_floor", "eval_window")]
